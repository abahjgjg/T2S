...
interface Props {
  ...
  onAskQuestion?: (question: string) => void; // New prop
}
...
export const TrendAnalysis: React.FC<Props> = ({
  ...
  // Performance Optimization: Memoize scatter plot data
  const scatterData = useMemo(() => {
    return trends.map(t => ({
      name: t.title,
      x: t.relevanceScore,
      y: t.growthScore || 50, // Default to 50 if missing from old cache
      z: t.impactScore || 50,
      sentiment: t.sentiment
    }));
  }, [trends]);
    ...
    <div className="animate-[fadeIn_0.3s_ease-out]">
      ...
      {activeTrend && (
        <TrendDeepDiveModal 
          ...
          uiText={uiText}
          onAskQuestion={onAskQuestion} // Pass through
        />
      )}
            ...
            {viewMode === 'matrix' && (
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <XAxis type="number" dataKey="x" name="Relevance" unit="%" stroke="#64748b" domain={[0, 100]}>
                    <Label value="Relevance" offset={-10} position="insideBottom" fill="#64748b" fontSize={10} />
                  </XAxis>
                  <YAxis type="number" dataKey="y" name="Growth" unit="%" stroke="#64748b" domain={[0, 100]}>
                    <Label value="Growth/Hype" angle={-90} position="insideLeft" fill="#64748b" fontSize={10} />
                  </YAxis>
                  ...
                  <Scatter name="Trends" data={scatterData} fill="#10b981" animationDuration={1000}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {trends.map((trend, idx) => (
          <div 
            key={idx} 
            className={`
              bg-slate-900 border rounded-xl p-5 transition-all relative overflow-hidden flex flex-col group cursor-pointer
              ${selectedTrendIndices.has(idx) ? 'border-emerald-500/50 shadow-[0_0_15px_-5px_rgba(16,185,129,0.2)]' : 'border-slate-800 opacity-70 hover:opacity-100'}
            `}
            onClick={(e) => ideasLength === 0 && onToggleSelection(e, idx)}
          >
            {/* Selection Checkbox */}
            {ideasLength === 0 && (
            ...
            <div className="mb-4 pr-8">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2 mb-3">
                 <div className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 border ${
                   trend.sentiment === 'Positive' ? 'bg-emerald-950/30 text-emerald-400 border-emerald-500/20' :
                   trend.sentiment === 'Negative' ? 'bg-red-950/30 text-red-400 border-red-500/20' :
                   'bg-blue-950/30 text-blue-400 border-blue-500/20'
                 }`}>
                    {getSentimentIcon(trend.sentiment)}
                    {trend.sentiment || 'News'} Signal
                 </div>
                 {trend.date && (
                   <div className="flex items-center gap-1 text-[10px] text-slate-500 font-mono">
                     <Calendar className="w-3 h-3" /> {trend.date}
                   </div>
                 )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="flex flex-col gap-1">
                   <div className="flex justify-between text-[10px] text-slate-500 uppercase font-bold">
                      <span>Velocity</span>
                      <span>{trend.growthScore || 50}%</span>
                   </div>
                   <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                      <div className="h-full bg-purple-500" style={{ width: `${trend.growthScore || 50}%` }}></div>
                   </div>
                </div>
                <div className="flex flex-col gap-1">
                   <div className="flex justify-between text-[10px] text-slate-500 uppercase font-bold">
                      <span>Impact</span>
                      <span>{trend.impactScore || 50}%</span>
                   </div>
                   <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                      <div className="h-full bg-indigo-500" style={{ width: `${trend.impactScore || 50}%` }}></div>
                   </div>
                </div>
            </div>
