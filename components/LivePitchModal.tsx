         ...
         {availablePersonas.map(persona => (
           <div key={persona.id} onClick={() => setSelectedPersona(persona)} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && setSelectedPersona(persona)} className={`p-4 rounded-xl border cursor-pointer transition-all flex items-start gap-4 ${selectedPersona.id === persona.id ? 'bg-emerald-900/20 border-emerald-500 ring-1 ring-emerald-500/50' : 'bg-slate-950/50 border-slate-800 hover:border-slate-700 hover:bg-slate-900'}`}>
              ...
              <div className="flex-1">
                 <div className="flex justify-between items-center mb-1"><div className="flex items-center gap-2"><h3 className={`font-bold ${selectedPersona.id === persona.id ? 'text-white' : 'text-slate-300'}`}>{persona.name}</h3>{persona.customData && (<span className={`${FONT_SIZES.micro} bg-indigo-500/20 text-indigo-300 px-1.5 py-0.5 rounded border border-indigo-500/30 font-mono`}>GENERATED</span>)}</div><span className={`${FONT_SIZES['2xs']} uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-slate-700`}>{persona.role}</span></div>
        ...
        <div className="flex flex-col items-center mb-6 shrink-0">
         ...
         <div className={`flex-1 bg-slate-950/50 border border-slate-800 rounded-xl p-4 overflow-y-auto mb-4 min-h-[${DIMENSIONS.livePitch.transcriptMinHeight}px] relative scrollbar-thin scrollbar-thumb-slate-700`}>
           {transcripts.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-slate-600"><MessageSquare className="w-8 h-8 mb-2 opacity-20" /><p className="text-xs">Waiting for audio...</p></div> : (
             <div className="space-y-3">{transcripts.map((t, i) => (<div key={i} className={`flex ${t.isUser ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[${DIMENSIONS.livePitch.transcriptMaxWidth}%] rounded-lg px-3 py-2 text-xs leading-relaxed shadow-sm ${t.isUser ? 'bg-blue-900/40 text-blue-100 border border-blue-500/20' : 'bg-emerald-900/30 text-emerald-100 border border-emerald-500/20'}`}><span className={`block font-bold opacity-50 ${FONT_SIZES['2xs']} mb-0.5 uppercase tracking-wide ${t.isUser ? 'text-blue-300' : 'text-emerald-300'}`}>{t.isUser ? 'You' : selectedPersona.name}</span>{t.text}</div></div>))}<div ref={transcriptEndRef} /></div>
           )}
        </div>
