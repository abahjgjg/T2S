          ...
          <div className="flex justify-between items-center mb-8">
             ...
             {!isGeneratingBlueprint && (
               <div className="flex bg-slate-900 p-1 rounded-lg border border-slate-800">
                 <button onClick={() => setViewMode('grid')} className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-slate-800 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`} title="Grid View"><LayoutGrid className="w-4 h-4" /></button>
                 <button onClick={() => setViewMode('compare')} className={`p-2 rounded-md transition-all ${viewMode === 'compare' ? 'bg-slate-800 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`} title="Comparison Table"><Table className="w-4 h-4" /></button>
               </div>
             )}
          </div>
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {ideas.map((idea) => (
                       <tr key={idea.id} className="hover:bg-slate-800/30 transition-colors">
                          ...
                          <td className="p-4"><button onClick={() => onSelectIdea(idea)} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-bold whitespace-nowrap">Build</button></td>
                       </tr>
                    ))}
                  </tbody>
                </table>
                ...
                return (
                  <div key={idea.id} className={`group relative bg-slate-900 border rounded-2xl p-6 transition-all hover:shadow-[0_0_30px_-5px_rgba(16,185,129,0.2)] flex flex-col ${isCached ? 'border-indigo-500/30 hover:border-indigo-500/60' : 'border-slate-800 hover:border-emerald-500/50'}`}>
                    ...
                    <div className="space-y-3 mb-6">
                      <div className="flex items-center gap-2 text-sm text-slate-300"><DollarSign className={`w-4 h-4 ${isCached ? 'text-indigo-500' : 'text-emerald-500'}`} /><span>{idea.potentialRevenue}</span></div>
                       <div className="flex items-center gap-2 text-sm text-slate-300"><Target className="w-4 h-4 text-blue-500" /><span>{uiText.model}: {idea.monetizationModel}</span></div>
                    </div>
                    <button onClick={() => onSelectIdea(idea)} className={`w-full py-3 font-bold rounded-lg transition-colors flex justify-center items-center gap-2 mt-auto ${isCached ? 'bg-indigo-600 hover:bg-indigo-500 text-white' : 'bg-white text-slate-950 hover:bg-emerald-400'}`}>
                      {isCached ? <>{uiText.viewBlueprint} <ArrowRight className="w-4 h-4" /></> : <>{uiText.buildBlueprint} <Zap className="w-4 h-4" /></>}
                    </button>
                  </div>
                );
              })}
